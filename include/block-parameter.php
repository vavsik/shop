...
$result = mysql_query("SELECT * FROM table_products WHERE kind = 'benzopila'",$link);
...
$row = mysql_fetch_array($result);
 ...
 if ($_GET["brand"])
 {
    if (in_array($row["kind_id"],$_GET["brand"]))
    {
...
<li><input '.$checked_brand.' type="checkbox"name="brand[]" value="'.$row["_kind_id"].'" id="checkbrend'.$row["kind_id"].'" /><label for="checkbrend'.$row["kind_id"].'">'.$row["brand"].'</label></li>
